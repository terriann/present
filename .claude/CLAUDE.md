...
- **Prefer black/white with opacity over gray.** When a subtle tint or background is needed, use `Color.black.opacity(x)` / `Color.white.opacity(x)` (or the light/dark adaptive `Color(light:dark:)` initializer) instead of `Color.gray.opacity(x)`. Black and white adapt predictably across palettes and appearance modes; gray introduces an intermediate tone that can drift from the intended look. See `Constants.alternatingRowBackground` for the canonical pattern.
