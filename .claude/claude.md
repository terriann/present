...
- **Standard CRUD verbs**: `add` (create), `get` (read one), `list` (read many), `update` (modify), `delete` (remove). Domain-specific verbs for specialized actions: `start`, `stop`, `pause`, `resume`, `cancel`, `search`, `archive`, `unarchive`, `tag`, `untag`, `note`, `export`.
- **Cross-noun actions live under the subject noun**: Tagging an activity is `activity tag <id> <tagId>` (not `tag add`). `tag add` creates a new tag.
- **File structure**: `Commands/<Noun>/<Noun><Verb>Command.swift`. Each noun group has a parent `<Noun>Command.swift` that registers subcommands.
- **Naming**: Structs are `<Noun><Verb>Command` (e.g., `SessionStartCommand`), file name matches struct name.
